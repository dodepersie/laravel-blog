...
<main class="container max-w-screen-sm md:max-w-screen-md lg:max-w-screen-xl mt-24 mx-auto overflow-x-hidden">
  ...
  <h1 class="my-4 md:my-7 text-4xl md:text-5xl lg:text-6xl font-extrabold text-gray-900 dark:text-white leading-relaxed"><span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">{{ __('category.categories') }}</span> {{ __('category.categories_y') }}</h1>
  
  <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-max">
      @foreach ($categories->sortBy('name') as $category)
      <figure class="relative max-w-screen-sm transition-all duration-300 cursor-pointer filter grayscale hover:grayscale-0">
        <a href="{{ '/' . app()->getLocale() . '/posts?category=' . $category->slug}}">
          <img class="rounded-lg" src="https://source.unsplash.com/600x600?{{ $category->name }}" alt="{{ $category->name }}">
        </a>
